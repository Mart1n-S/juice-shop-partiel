...
export const publicKey = fs.readFileSync(
  path.resolve(process.cwd(), 'jwt.pub'),
  'utf8'
)

const privateKey = fs.readFileSync(
  path.resolve(process.cwd(), 'jwt.key'),
  'utf8'
)
...
export const hmac = (data: string) =>
  crypto
    .createHmac('sha256', requireEnv('HMAC_SECRET'))
    .update(data)
    .digest('hex')
...
export const deluxeToken = (email: string) => {
  const secret = requireEnv('DELUXE_TOKEN_SECRET')
  return crypto
    .createHmac('sha256', secret)
    .update(email + roles.deluxe)
    .digest('hex')
}
